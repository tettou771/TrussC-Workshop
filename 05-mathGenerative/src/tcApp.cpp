...
void tcApp::draw() {
    ...
    // =========================================================
    // 챌린지:
    //   noise를 사용해서 파티클 플로우 필드를 만들어보자
    //   1. tcApp.h에서 vector<Vec2> particles를 만들고 랜덤 위치로 초기화
    //   2. update()에서 각 파티클의 위치로부터 noise로 각도를 가져옴:
    //      float angle = noise(p.x * 0.005, p.y * 0.005, time) * TAU;
    //   3. 그 각도 방향으로 약간 이동:
    //      p.x += cos(angle) * 2;  p.y += sin(angle) * 2;
    //   4. draw()에서 작은 점으로 그리기
    //   5. 화면 밖으로 나가면 랜덤으로 리셋
    //
    //   noiseField2dExample도 참고해봐!
    // =========================================================
